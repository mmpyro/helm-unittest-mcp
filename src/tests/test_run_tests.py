...
def test_run_unittest_internal_success(
    ...
    mock_run.assert_called_once_with(
        expected_cmd, text=True, capture_output=True, check=False
    )
...
@patch("tools.run_tests.os.path.exists")
@patch("tools.run_tests.os.remove")
def test_run_unittest_cleanup_on_error(
    mock_remove, mock_exists, mock_close, mock_run, mock_parser_class
):
...
def test_run_unittest_test_case_filtering_and_truncation(
    ...
    msg = result_failed_only.test_cases[0].message
    assert msg is not None
    assert msg.startswith("A" * 100)
    assert "... [truncated 1400 chars]" in msg

    # Test "none"
    mock_summary.test_cases = [
        TestCaseResult(name="pass_1", suite="S1", result="Pass", time=0.1),
        TestCaseResult(
            name="fail_1", suite="S1", result="Fail", time=0.1, message="Err"
        ),
    ]
