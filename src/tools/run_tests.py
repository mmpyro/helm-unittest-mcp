...
def _run_unittest_internal(
    ...
    try:
        parser = TestResultParser(output_type)
...
@mcp.tool()
def run_unittest(test_suite_files: str, chart_path: str, values_path: list[str] = [], output_type: str = "xunit", output_file: Optional[str] = None) -> TestResultSummary:
    ...
    Args:
        ...
        output_file (str, optional): Path where to save the test report. If not provided, a temporary file will be used.
        
    Returns:
        TestResultSummary: A summary of the test execution, including total counts and individual test cases.
    """
    return _run_unittest_internal(test_suite_files, chart_path, values_path, output_type, output_file, update_snapshot=False)


@mcp.tool()
def update_snapshot(test_suite_files: str, chart_path: str, values_path: list[str] = [], output_type: str = "xunit", output_file: Optional[str] = None) -> TestResultSummary:
    ...
    Returns:
        TestResultSummary: A summary of the test execution, including total counts and individual test cases.
    """
    return _run_unittest_internal(test_suite_files, chart_path, values_path, output_type, output_file, update_snapshot=True)
